...
export default function CreateLot() {
  ...
  const onSubmit = async (data: CreateLotData) => {
    ...
    try {
      // Map the React form data to match the FastAPI Pydantic schema perfectly
      const payload = {
        material_category_id: data.category_id, 
        material_category_name: data.category_id, 
        material_description: data.description || `Condition: ${data.condition}`,
        approximate_weight: Number(data.weight_kg),
        
        // Default location data since the form doesn't collect it yet
        collection_location: "Location not provided", 
        latitude: 0.0,
        longitude: 0.0
      };

      const lotResponse = await lotApi.createLot(payload);
      
      // Extract the ID safely based on the backend's APIResponse wrapper
      const lotId = lotResponse.data?.data?.id || lotResponse.data?.id;

      if (!lotId) {
        throw new Error("Server did not return a valid Lot ID.");
      }

      await lotApi.uploadLotImage(lotId, imageFile);
      
      // Force a tiny delay so Firestore can settle before redirecting to dashboard
      setTimeout(() => {
        navigate('/collector/dashboard');
      }, 500);

    } catch (err: any) {
      const errorMessage = err.response?.data?.detail || err.message || 'Failed to create lot. Please try again.';
...
//     try {
//       // Explicitly parse weight_kg as a number to prevent 422 type mismatches
//       const payload = {
//         ...data,
